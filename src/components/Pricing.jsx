  ...
  return (
    <section id="pricing" aria-labelledby="pricing-title" className="mx-auto max-w-6xl px-6 py-24">
      <header className="mx-auto mb-12 max-w-2xl text-center">
        <p className="mb-3 font-display text-sm font-semibold uppercase tracking-widest text-saffron">Pricing</p>
        <h2 id="pricing-title" className="font-display text-3xl font-bold tracking-tight md:text-5xl">
        ...
        <div className="mt-8 flex flex-col items-center gap-4">
          ...
          <div role="group" aria-label="Billing cycle" className="inline-flex rounded-full border border-line bg-elevated p-1">
            ...
            <span
              id="annual-pill"
              className="pointer-events-none ml-2 self-center rounded-full bg-saffron/15 px-2.5 py-1 font-display text-xs font-semibold text-saffron opacity-0 transition-opacity duration-200 ease-micro"
            >
              Save 20%
            </span>
          </div>
          ...
          >
            ...
            <div className="mt-5 flex items-baseline gap-1">
              ...
              <span
                ...
                className="font-display text-4xl font-bold tabular-nums text-powder"
                aria-live="polite"
              >
            ...
            <button
              className={`mt-6 rounded-xl px-4 py-2.5 text-sm font-semibold transition-colors duration-200 ease-micro ${
                tier.highlight ? 'bg-accent text-ink hover:brightness-105' : 'border border-line text-powder hover:border-accent'
              }`}
            >
              Start free trial
            </button>
            <ul className="mt-6 space-y-3 text-sm">
              {tier.features.map((f) => (
                <li key={f} className="flex items-start gap-2 text-muted">
                  <Icon name="chevronRight" size={16} className="mt-0.5 shrink-0 text-accent" />
                  <span>{f}</span>
                </li>
              ))}
            </ul>
